...
public class NewCommentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			info ="repeat_submit_comment!";
		}
				
		//发送新的cookie
		//Cookie c = new Cookie(cookie_name,DateUtils.getFormatDate(new Date()));
		Cookie c = new Cookie(cookie_name,"2018-5-12");
		c.setMaxAge(60 * 60);
		c.setPath("/servlet");
		response.addCookie(c);
		
		request.setAttribute("info", info);
		//request.getRequestDispatcher("/page.articles.jsp").forward(request, response);
